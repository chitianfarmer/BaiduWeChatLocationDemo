 ...
 * 类描述：BaiduSearchPositionAdapter 描述:poi地点列表适配器(搜索页面)
 * 创建人：songlijie
 * 创建时间：2018/6/6 17:37
 * 邮箱:814326663@qq.com
 */
public class BaiduSearchPositionAdapter extends RecyclerView.Adapter<BaiduSearchPositionAdapter.LocatorSearchViewHolder> {
    ...
    public BaiduSearchPositionAdapter(Context context, List<LocationBean> datas) {
        this.datas = datas;
    ...
    @Override
    public LocatorSearchViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = View.inflate(context, R.layout.location_item_poi, null);
        LocatorSearchViewHolder viewHolder = new LocatorSearchViewHolder(view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(LocatorSearchViewHolder holder, final int position) {
        final LocationBean info = datas.get(position);
        holder.tv_poi_address.setText(info.getCity() + info.getAddress());
    ...
    public class LocatorSearchViewHolder extends RecyclerView.ViewHolder {
        ...
        public LocatorSearchViewHolder(View itemView) {
            super(itemView);
